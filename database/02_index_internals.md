...
- **Leaf Nodes:** Chứa Key và dữ liệu thực tế (hoặc con trỏ tới dữ liệu). Các Leaf Nodes được **nối với nhau bằng Doubly Linked List**, giúp Range Scan cực kỳ hiệu quả mà không cần duyệt lại từ Root.
...
- **Bookmark Lookup (2 lần):** Khi truy vấn bằng Secondary Index, MySQL phải tìm trên Secondary Index lấy `Primary_Key`, sau đó dùng `Primary_Key` đó tra cứu lần 2 trên Clustered Index để lấy các cột còn lại.
...
| **Leaf Node của Clustered/Primary Index** | Chứa full row data | Chứa `(Index_Key, ctid)` |
...
**Điểm then chốt:** Để kích hoạt **Index-Only Scan**, Postgres bắt buộc phải kiểm tra **Visibility Map (VM)**. Nếu VM xác nhận Data Page là `ALL-VISIBLE` (tất cả tuple đã committed, không có dead tuple), Postgres mới bỏ qua Heap Page. Nếu chưa `ALL-VISIBLE`, Postgres vẫn phải đọc Heap Page để kiểm tra `xmin`/`xmax`.
