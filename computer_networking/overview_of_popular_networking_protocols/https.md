Tiếp nối các bài giảng về DNS (để tìm địa chỉ), TLS (để mã hóa) và Certificate (để xác thực), chúng ta sẽ ghép tất cả các mảnh lại để tạo nên giao thức nền tảng của web an toàn hiện đại: **HTTPS**.
...
Toàn bộ quá trình giao tiếp HTTP (như `GET /index.html`, gửi biểu mẫu, cookie...) sẽ được "bọc" trong một đường hầm an toàn được tạo bởi TLS.

---

# HTTPS: Giao thức truyền tải siêu văn bản an toàn

Khi bạn đã hiểu về **DNS** (để tìm địa chỉ), **TCP** (để mở kết nối), **TLS** (để mã hóa) và **Certificate** (để xác thực), chúng ta có thể ghép tất cả các mảnh lại để tạo nên giao thức nền tảng của web an toàn hiện đại: **HTTPS**.
...
-   [1. Ba trụ cột bảo mật của HTTPS](#1-ba-trụ-cột-bảo-mật-của-https)
-   [2. Quy trình hoạt động "Base" của HTTPS (Luồng đầy đủ)](#2-quy-trình-hoạt-động-base-của-https-luồng-đầy-đủ)
-   [3. Bước 1: Phân giải DNS (Tìm địa chỉ)](#3-bước-1-phân-giải-dns-tìm-địa-chỉ)
-   [4. Bước 2: Bắt tay TCP (Mở kết nối)](#4-bước-2-bắt-tay-tcp-mở-kết-nối)
-   [5. Bước 3: Bắt tay TLS (Xác thực & Trao đổi khóa)](#5-bước-3-bắt-tay-tls-xác-thực--trao-đổi-khóa)
-   [6. Bước 4: Giao tiếp HTTP an toàn (Truyền dữ liệu)](#6-bước-4-giao-tiếp-http-an-toàn-truyền-dữ-liệu)
